...
const TO_REMOVE_KEYS = [
  'propertyLocationBlock',
  'propertyLocationPhase',
  'propertyType',
];

export const FormEditProperty: React.FC<Props> = ({
  id,
  onCancel,
  onSuccess,
}) => {
  ...
  const form = useForm<PropertyCreateParams>({
    ...
    validate: joiResolver(editPropertySchema),
  });
  const {
    mutate: editProperty,
    reset,
    ...
    isSuccess,
    isError,
  } = useEditProperty(id, form.values);
  ...
  const isSubmitDisabled = React.useMemo(
    () =>
      getProperty?.data
        ? !hasDataChanges(getProperty?.data, form.values, TO_REMOVE_KEYS)
        : true,
    [form.values, getProperty?.data]
  );
  ...
  const handleFormSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (getProperty?.data.id && !Object.keys(form.errors).length) {
      editProperty();
    }
  };
  ...
  React.useEffect(() => {
    if (isSuccess) {
      onSuccess('Property edited successfully');
  ...
  React.useEffect(() => {
    if (!isDataMounted && getProperty?.data) {
      const neededData = objectKeyRemover(getProperty.data, TO_REMOVE_KEYS);
      form.setValues(neededData);

      setIsDataMounted(true);
    }
  }, [form, getProperty?.data, isDataMounted]);
    ...
    <>
      {isError && (
        <Alert
          color="red"
          mb={20}
          title="Encountered an error while editing property"
        >
